import chara from "../data/clara.json";
...
export function calc(info){
  ...
  skillResult['detail'] = [
      ...
      dmg : calcEnemy(info,skill[1],debuff,"add")
      }
  ]
  ultResult['detail'] = [
    {
  ...
  if(document.getElementById("add6").checked){
    ...
    talentResult['detail'] = [
      ...
      {
        name : "必殺技後カウンター(メインターゲット)",
        dmg : calcEnemy(info,talent[0]+ult[1]*info.atk/100,debuff,"atk")
      },
      {
        name : "必殺技後カウンター(隣接)",
        dmg : calcEnemy(info,(talent[0]+ult[1]*info.atk/100)/2,debuff,"atk")
      }
    ]
    info.dmgbuff = tmp;
  }else
  talentResult['detail'] = [
    ...
    {
      name : "必殺技後カウンター(隣接)",
      dmg : calcEnemy(info,(talent[0]+ult[1]*info.atk)/200,debuff,"atk")
    }
  ]
